...
const iconFor = (t: string) => {
  ...
  if (t.toLowerCase().includes("nutrition") || t.toLowerCase().includes("water") || t.toLowerCase().includes("hydr")) return Apple;
...
function Notifs() {
  ...
  useEffect(() => { const t = setTimeout(mark, 700); return () => clearTimeout(t); }, [mark]);
        ...
        {items.length === 0 ? (
          ...
          <ul className="mt-8 space-y-2">
            {items.map((n) => {
              ...
              return (
                <li key={n.id} className={`flex gap-3 rounded-2xl border px-5 py-4 ${n.read ? "border-border bg-surface" : "border-primary/30 bg-primary/5"}`}>
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-surface-elevated"><Icon className="h-4 w-4 text-primary" /></span>
                  <div className="min-w-0 flex-1">
                    <p className="text-[15px] font-medium">{n.title}</p>
                    <p className="mt-0.5 text-xs text-muted-foreground">{n.body}</p>
                    <p className="mt-1 text-[10px] uppercase tracking-[0.22em] text-muted-foreground/60">{formatDistanceToNow(n.at, { addSuffix: true })}</p>
                  </div>
                </li>
              );
            })}
