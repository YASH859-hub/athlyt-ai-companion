    ...
    <div className="mx-auto min-h-[100dvh] max-w-[480px] px-6 pb-10 pt-12">
      ...
      <Section title="Danger">
        <button
          ...
          className="flex w-full items-center justify-between px-5 py-4 text-left text-[15px] text-warning"
        >
          Reset app data <ChevronRight className="h-4 w-4 text-muted-foreground" />
        </button>
        <button
          onClick={signOut}
          className="flex w-full items-center justify-between px-5 py-4 text-left text-[15px] text-destructive"
        >
          Sign out <ChevronRight className="h-4 w-4 text-muted-foreground" />
        </button>
      </Section>

      <p className="mt-8 mb-4 text-center text-[10px] uppercase tracking-[0.32em] text-muted-foreground/60">
        ATHLYT · v1.0
      </p>
    </div>
  );
