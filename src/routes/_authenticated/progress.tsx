    ...
    <AppShell>
      <div className="px-6 pt-14">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {xp} XP earned · {xp % 500}/500 to next
        </p>
        ...
        <section className="mt-10">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full bg-surface px-3 py-1.5 text-xs"
            >
          ...
          <div className="mt-4 rounded-2xl border border-border bg-surface p-5">
            <p className="text-2xl font-semibold">
              {weights[weights.length - 1]?.kg.toFixed(1)} kg
            </p>
            <svg viewBox="0 0 300 100" className="mt-3 h-24 w-full">
              <polyline
                fill="none"
                stroke="oklch(0.94 0.22 124)"
                strokeWidth="2"
                points={weights
                  .map(
                    (p, i) =>
                      `${(i / (weights.length - 1)) * 300},${100 - ((p.kg - minW) / range) * 90}`,
                  )
                  .join(" ")}
              />
              {weights.map((p, i) => (
                <circle
                  key={i}
                  cx={(i / (weights.length - 1)) * 300}
                  cy={100 - ((p.kg - minW) / range) * 90}
                  r={2.5}
                  fill="oklch(0.94 0.22 124)"
                />
              ))}
            </svg>
          </div>
          ...
          <div className="mt-4 grid grid-cols-14 gap-1">
            {Array.from({ length: 14 }).map((_, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`h-7 rounded ${done ? "bg-primary" : "bg-surface-elevated"}`}
                />
              );
            })}
          </div>
        ...
        <section className="mt-8 mb-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {["Week 1", "Today"].map((l) => (
              <div
                key={l}
                className="aspect-[3/4] rounded-2xl border border-dashed border-border bg-surface/50 grid place-items-center text-xs text-muted-foreground"
              >
                {l}
              </div>
            ))}
          </div>
          <button className="mt-3 w-full rounded-2xl border border-border bg-surface py-3 text-sm">
            + Add photo
          </button>
        </section>

        <Link to="/" className="mt-4 mb-8 block text-center text-xs text-muted-foreground">
          Back to today →
        </Link>
      </div>
          ...
          <div className="w-full max-w-sm rounded-3xl border border-border bg-surface p-6">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="grid h-8 w-8 place-items-center rounded-full bg-surface-elevated"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <button
              ...
              className="mt-4 w-full rounded-full bg-primary py-3 font-semibold text-primary-foreground"
            >
              Save
            </button>
          </div>
