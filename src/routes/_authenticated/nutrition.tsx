...
function Nutrition() {
  ...
  const meals = useApp((s) =>
    s.meals.filter((m) => new Date(m.at).toDateString() === new Date().toDateString()),
  );
  ...
  return (
    <AppShell>
      <div className="px-6 pt-14">
        ...
        <h1 className="mt-3 text-[34px] font-semibold tracking-tight">
          {today.kcalIn}{" "}
          <span className="text-muted-foreground text-[20px]">/ {targets.kcal} kcal</span>
        </h1>
        ...
        <section className="mt-6 rounded-2xl border border-border bg-surface p-5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">
              {today.waterMl} / {targets.water} ml
            </span>
          </div>
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-surface-elevated">
            <div
              className="h-full bg-secondary"
              style={{ width: `${Math.min(100, (today.waterMl / targets.water) * 100)}%` }}
            />
          </div>
