        ...
        <div className="mt-8 space-y-3">
          {WORKOUTS.map((w) => (
            <button
              key={w.id}
              onClick={() => setActive(w)}
              className="flex w-full items-center justify-between rounded-2xl border border-border bg-surface px-5 py-4 text-left"
            >
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {w.duration} min · {w.exercises.length} exercises · Lv {w.level}
                </p>
              </div>
...
function Player({ workout, onExit }: { workout: Workout; onExit: () => void }) {
  ...
  useEffect(() => {
    if (resting && rest === 0) setResting(false);
  }, [rest, resting]);
    ...
    return (
      <div className="mx-auto grid min-h-[100dvh] max-w-[480px] place-items-center bg-background px-6">
        <div className="text-center">
          <div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-primary/15">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {workout.name} · {Math.floor(elapsed / 60)}m {elapsed % 60}s
          </p>
          ...
          <div className="mt-8 flex flex-col gap-2">
            ...
            <Link to="/progress" className="text-sm text-muted-foreground">
              See progress →
            </Link>
          </div>
        </div>
    ...
    <div className="mx-auto flex min-h-[100dvh] max-w-[480px] flex-col bg-background px-6 pb-10 pt-12">
      ...
      <div className="mt-6 flex gap-1.5">
        {workout.exercises.map((_, idx) => (
          <div
            key={idx}
            className={`h-1 flex-1 rounded-full ${idx < i ? "bg-primary" : idx === i ? "bg-foreground/60" : "bg-surface-elevated"}`}
          />
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {resting ? (
          ...
          >
            ...
            <p className="mt-6 text-[88px] font-semibold leading-none tracking-tight text-primary">
              {rest}
            </p>
            <p className="mt-4 text-sm text-muted-foreground">Next: {workout.exercises[i]?.name}</p>
            <button
              onClick={() => setResting(false)}
              className="mt-10 rounded-full bg-foreground px-6 py-3 text-sm font-semibold text-background"
            >
              Skip rest
            </button>
          </motion.div>
          ...
          >
            ...
            <div className="mt-8 flex items-baseline gap-6">
              <div>
                ...
                <p className="text-4xl font-semibold">{ex.sets}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.24em] text-muted-foreground">
                  Reps
                </p>
                <p className="text-4xl font-semibold">{ex.reps}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.24em] text-muted-foreground">
                  Rest
                </p>
                <p className="text-4xl font-semibold">{ex.rest}s</p>
              </div>
            </div>
            <div
              className="mt-12 h-44 rounded-3xl bg-surface"
              style={{
                background:
                  "radial-gradient(120% 80% at 80% 20%, rgba(139,92,246,0.25), transparent 60%), radial-gradient(80% 100% at 10% 100%, rgba(199,255,47,0.12), transparent 60%), #121212",
              }}
            />
          </motion.div>
        )}
      ...
      <button
        onClick={next}
        className="mt-6 w-full rounded-full bg-primary py-4 text-[15px] font-semibold text-primary-foreground"
      >
