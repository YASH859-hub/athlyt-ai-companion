    ...
    <div className="mx-auto min-h-[100dvh] max-w-[480px] px-6 pb-10 pt-12">
      ...
      <form onSubmit={submit} className="mt-10 space-y-4">
        <label className="block">
          ...
          <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@athlyt.com"
        ...
        <label className="block">
          <span className="text-[11px] uppercase tracking-[0.24em] text-muted-foreground">Password</span>
          <input type="password" required value={pw} onChange={(e) => setPw(e.target.value)} placeholder="••••••••"
            className="mt-2 w-full rounded-2xl border border-border bg-surface px-4 py-4 text-[15px] outline-none focus:border-foreground/40" />
        </label>

        <div className="flex justify-end pt-1">
          <Link to="/auth/forgot" className="text-[13px] text-muted-foreground">Forgot password?</Link>
        </div>
        ...
        <p className="pt-2 text-center text-[13px] text-muted-foreground">
          New here?{" "}<Link to="/auth/signup" className="font-semibold text-foreground">Create an account</Link>
        </p>
      </form>
    </div>
  );
